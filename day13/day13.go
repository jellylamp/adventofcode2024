...
func PartB(filename string) int {
	...
	for index := 0; index < len(lineArr); index = index + 4 {
		...
		prizeString := lineArr[index+2]
		prizeNums := re.FindAllString(prizeString, -1)
		prizeX := utils.ConvertStringToInt(prizeNums[0]) + 10000000000000
		prizeY := utils.ConvertStringToInt(prizeNums[1]) + 10000000000000

		// feed values into BST - once we have equaled or gone past X and Y values, then we should break that chain.
		// If we do find an equal, add to map and we will calculate smallest amount of tokens later
		binarySearchTreeDFSPartB(prizeX, prizeY, 0, 0, 0, 0, buttonA, buttonB)
		...
		for key := range countMap {
			// a costs 3
			// b costs 1
			tokenTotal := (key.XModifier)*3 + (key.YModifier)*1
			if tokenTotal < lowestTokenTotal || lowestTokenTotal == 0 {
				lowestTokenTotal = tokenTotal
			}
		}
...
func binarySearchTreeDFSPartB(targetX, targetY, currentADepth, currentBDepth, runningX, runningY int, buttonA, buttonB XYModifiers) bool {
	// Generate a unique key for the current state
	stateKey := fmt.Sprintf("%d,%d,%d,%d", runningX, runningY, currentADepth, currentBDepth)
	...
	// Recursive calls: press button A or button B
	pressA := binarySearchTreeDFSPartB(targetX, targetY, currentADepth+1, currentBDepth, runningX+buttonA.XModifier, runningY+buttonA.YModifier, buttonA, buttonB)
	pressB := binarySearchTreeDFSPartB(targetX, targetY, currentADepth, currentBDepth+1, runningX+buttonB.XModifier, runningY+buttonB.YModifier, buttonA, buttonB)
